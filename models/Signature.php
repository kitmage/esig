...
class WP_E_Signature extends WP_E_Model {
    ...
    public function esign_set_json($user_id, $csum_id, $owner_id = false,$signature_type=false) {
        ...
        {
            // Remove any whitespace, line breaks, or spaces from base64 data to prevent broken images
        ...
        // Attempt to write JSON safely; avoid fwrite on invalid handle
        $written = @file_put_contents($file_name, (string) $json, LOCK_EX);
        if ($written === false) {
            $sigfile = @fopen($file_name, "w");
            if (is_resource($sigfile)) {
                @fwrite($sigfile, (string) $json);
                @fclose($sigfile);
            } else {
                // Log but do not fatal; upstream will fallback to legacy image path
                error_log('ESIG: Failed to open signature temp file for writing: ' . $file_name);
            }
        }
    ...
    public function display_owner_signature($signature_id) {
        ...
        if ($written === false) {
            ...
            if (is_resource($sigfile)) {
                ...
                error_log('ESIG: Failed to open owner signature temp file for writing: ' . $file_name);
            }
        }
