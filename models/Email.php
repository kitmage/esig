...
class WP_E_Email extends WP_E_Model {
    ...
    public function default_mail($to_email, $subject, $message, $headers, $attachments = false, $wpheaders=false,$senderHeader=array()) {
        ...
        add_action('wp_mail_failed', array($this, 'log_wp_mail_error'), 10, 1);
        ...
        if ($attachments) {
            ...
            try {  
                if (!file_exists($attachments) || !is_readable($attachments)) {
                    $this->maybe_log_mail_error('Attachment file not found or not readable: ' . $attachments);
                    remove_filter('wp_mail_content_type', 'set_html_content_type');
                    remove_action('wp_mail_failed', array($this, 'log_wp_mail_error'), 10);
                    return false;
                }
                
                // wp_mail expects attachments as a string path or array of paths
                // For Brevo/WP Mail SMTP compatibility, ensure proper format
                $attachment_path = $attachments;
                if (!is_array($attachment_path)) {
                    // Wrap single attachment in array for consistent handling
                    $attachment_path = array($attachments);
                }
                
                $mailsent = wp_mail($to_email, $subject, $message, $headers, $attachment_path);
                
                if (!$mailsent) {
                    $this->maybe_log_mail_error('wp_mail returned false for email to ' . $to_email . ' with attachment');
                }
            } catch (Exception $e) {
                $this->maybe_log_mail_error('Mail exception: ' . $e->getMessage());
                $mailsent = false;
            }
        } else {
            try {
                ...
                if (!$mailsent) {
                    $this->maybe_log_mail_error('wp_mail returned false for email to ' . $to_email);
                }
            } catch (Exception $e) {
                $this->maybe_log_mail_error('Mail exception: ' . $e->getMessage());
                $mailsent = false;
    ...
    public function log_wp_mail_error($error) {
        if (is_wp_error($error)) {
            $this->maybe_log_mail_error('wp_mail failed: ' . $error->get_error_message());
            $error_data = $error->get_error_data();
            if (!empty($error_data)) {
                $this->maybe_log_mail_error('wp_mail error data: ' . print_r($error_data, true));
            }
        }
    }

    private function maybe_log_mail_error($message) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            $log_path = trailingslashit(WP_CONTENT_DIR) . 'debug.log';
            error_log('[WP E-Signature] ' . $message . PHP_EOL, 3, $log_path);
        }
    }
    ...
    private function get_default_data($template_data = array(), $wpUserId=false) {
