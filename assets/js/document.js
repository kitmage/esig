...
//"use strict";
    ...
    $('body').on('click', '.invitations-container a', function (e) {
        ...
        $.post(esigAjax.ajaxurl + "?action=wp_e_signature_ajax&className=WP_E_Signer&method=display_signers&_wpnonce=" + esigAjax._wpnonce, {document_id: document_id}).done(function (data) {
    ...
    // when view input field focus out 
    $('body').on('keyup', "#recipient_emails input[name='cc_recipient_fnames\\[\\]']", function () {
        $('.esig-error-box').remove();
    });
    $('body').on('keyup', "#recipient_emails input[name='cc_recipient_emails\\[\\]']", function () {
        $('.esig-error-box').remove();
    });


    // view basic signer add submit form . 
    $("#esig-view-form").on("submit", function (e) {
        ...
        if (esign.ccValidate('#cc_recipient_emails', '.cc_recipient_emails_container')) {
            return false;
        }
        return true;
    });
