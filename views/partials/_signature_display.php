...
<div class="col-sm-6" <?php if (array_key_exists('esig-tooltip', $data)) {
							echo $data['esig-tooltip'];
						} ?>>
	<div style="pointer-events: none;" class="signature-wrapper-displayonly-signed" id="signature-<?php echo $data['user_id']; ?>">
		...
		<div class="signed rtl-signed">
			...
			if (isset($data['signature']) && $data['signature'] == "yes") {
				...
				$imageSize = $esignature->getSignatureWidthHeight($signature_image);
				echo '<img class="signature-image" src="' . $signature_image . '" width="'. $imageSize['width'] .'px" height="'. $imageSize['height'] .'px">';
				...
				echo '<img class="signature-image" src="' . $signature_image . '" >';
			}
		...
		<input type="hidden" name="esignature_in_text_signed" maxlength="64" class="esignature-in-text-signed" value="<?php if (array_key_exists('output_type', $data)) {
																															echo $data['output_type'];
																														} ?>" placeholder="Type signature">
		<input type="hidden" name="font_type_signed" class="font-type-signed" value="<?php if (array_key_exists('font_type', $data)) {
																							echo $data['font_type'];
																						} ?>">
	...
	<div class="signature-meta <?php echo $class; ?> rtl-signature-meta">
		<p>
			<?php if (array_key_exists('esig-awaiting-sig', $data)) {
				echo $data['esig-awaiting-sig'];
			} ?>
			<?php if (array_key_exists('by_line', $data)) {
				echo $data['by_line'] . ' ' . $data['user_name'];
			} ?><br />

			<?php if (array_key_exists('sign_date', $data)) {
				echo $data['sign_date'];
			} ?>
