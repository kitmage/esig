...
class WP_E_Shortcode {
    ...
    public function e_sign_document() {
        ...
        // Don't execute shortcode during REST API requests
        if (defined('REST_REQUEST') && REST_REQUEST) {
            return '';
        }
        
        // Check for REST API query parameter
        if (isset($_GET['rest_route']) || (isset($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], '/wp-json/') !== false)) {
            return '';
        }
        ...
        if (!ESIG_POST('recipient_signature') && !ESIG_POST('esignature_in_text')) {
            ...
            $invite_hash = WP_E_Sig()->validation->esig_clean(esigget("invite"));
            $checksum =  WP_E_Sig()->validation->esig_clean(esigget("csum"));
            ...
            if (empty($invite_hash) || empty($checksum)) {
                ...
                if ($esigPreview) {
                    ...
                    $template_data = array(
                        "message" => sprintf(__("<p align='center' class='esig-not-found-page-template'><img src='%s/images/logo.png' alt='Sign Documents Online using WordPress E-Signature by Approve Me'></a></p><p align='center' class='esig-not-found-page-template'><strong>Woah tiger!</strong> You are attempting to access <strong>" . esigget("document_title",$document) . "</strong> which was created by another user. For security reasons only the original document creator and recipients have access to this document. If you have questions about this document you can contact the document creator directly. <br>Thank you for using Wordpress Digital E-Signature By <a href='https://www.approveme.com/wp-e-signature/?e-signature-document' rel='nofollow' title='Document Signing using WP E-Signature by Approve Me'>Approve Me</a></p> <p align='center'><img src='%s/images/search.svg' alt='esignature by Approve Me' class='esig-not-found-search'></p>", 'esig'), $assets_dir,$assets_dir),
                    );
                } else {
                    
                    $search_img = apply_filters("esig_default_page_search_image" , sprintf(__("%s/images/search.svg","esig"),$assets_dir));
                    
                    $esig_doc_not_found_msg = sprintf(__("<p align='center' class='esig-not-found-page-template'><a href='https://www.approveme.com/wp-e-signature/?e-signature-document' title='Wordpress Digital E-Signature' rel='nofollow' target='_blank'><img src='%s/images/logo.png' alt='Sign Documents Online using WordPress E-Signature by Approve Me'></a></p><p align='center' class='esig-not-found-page-template'>Well this is embarrassing, but we can't seem to locate the document you're looking to sign online.<br>You may want to send an email to the website owner. <br>Thank you for using Wordpress Digital E-Signature By <a href='https://www.approveme.com/wp-e-signature/?e-signature-document' rel='nofollow' title='Document Signing using WP E-ESignature by Approve Me'>Approve Me</a></p> <p align='center'><img src='%s' alt='esignature by Approve Me' class='esig-not-found-search'><br><a class='esig-not-found-btn' href='https://www.approveme.com/wp-e-signature/?e-signature-document' rel='nofollow'>Get WP E-Signature!</a></p>", 'esig'), $assets_dir, $search_img);
                    
                    $template_data = array(
                        "message" => apply_filters("esig_document_default_message" , $esig_doc_not_found_msg)  //sprintf(__("<p align='center' class='esig-not-found-page-template'><a href='https://www.approveme.com/wp-e-signature/?e-signature-document' title='Wordpress Digital E-Signature' rel='nofollow' target='_blank'><img src='%s/images/logo.png' alt='Sign Documents Online using WordPress E-Signature by Approve Me'></a></p><p align='center' class='esig-not-found-page-template'>Well this is embarrassing, but we can't seem to locate the document you're looking to sign online.<br>You may want to send an email to the website owner. <br>Thank you for using Wordpress Digital E-Signature By <a href='/wp-e-signature/?e-signature-document' rel='nofollow' title='Document Signing using WPESignature by Approve Me'>Approve Me</a></p> <p align='center'><img src='%s/images/search.svg' alt='esignature by Approve Me' class='esig-404-search'><br><a class='esig-404-btn' href='/wp-e-signature/?e-signature-document' rel='nofollow'>Get WP E-Signature!</a></p>", 'esig'), $assets_dir, $assets_dir),
                    );
                }
                    //return
            ...
            if (WP_E_Sig()->document->document_exists($document_id) == 0  || WP_E_Sig()->document->getStatus($document_id) =="trash") {
                $template_data = array(
                    "message" => sprintf(__("<p align='center' class='esig-not-found-page-template'><a href='https://www.approveme.com/wp-e-signature/?e-signature-document' title='Wordpress Digital E-Signature using WP E-Signature by Approve Me' target='_blank' rel='nofollow'><img src='%s/images/logo.png' alt='Sign Documents Online using WordPress E-Signature by Approve Me'></a></p><p align='center' class='esig-not-found-page-template'>Well this is embarrassing, but we can't seem to locate the document you're looking to sign online.<br>You may want to send an email to the website owner. <br>Thank you for using Wordpress Digital E-Signature By <a href='https://www.approveme.com/wp-e-signature/?e-signature-document' title='Document Signing using WP E-Signature by Approve Me' rel='nofollow'>Approve Me</a></p> <p align='center'><img src='" . $assets_dir . "/images/search.svg' alt='esignature by Approve Me' class='esig-not-found-search'><br><a class='esig-not-found-btn' href='https://www.approveme.com/wp-e-signature/?e-signature-document' rel='nofollow'>Get WP E-Signature!</a></p>", 'esig'), $assets_dir),
                );
                $this->displayDocumentToSign(null, '404', $template_data);
            ...
            if ($this->doc_signature_validates()) {
                ...
                // check for both type and full signature not empty  
                if(empty($recipient_signature) && empty($esignature_in_text))
                {
                     WP_E_Sig()->notice->set('e-sign-red-alert email', 'Signature is empty');
                     error_log("Shortcode: e_sign_document: Signature is empty");
                ...
                $recipient_signature = ESIG_POST('recipient_signature');
                //check recipient signature validation 
                if (!empty($recipient_signature) && !WP_E_Sig()->validation->valid_json($recipient_signature)) {
                     
                     WP_E_Sig()->notice->set('e-sign-red-alert email', 'Signature is not valid');
                     error_log("Shortcode: e_sign_document: Signature is not valid");
                     wp_redirect(esc_url($_SERVER['HTTP_HOST'] . $_SERVER["REQUEST_URI"]));
                     exit;
    ...
    public function get_footer_ajax() {
        ...
        $template_data = array(
            ...
            'mode' => $mode,
            'cc_user_preview' => esigget('ccpreview')
        );
    ...
    public function displayDocumentToSign($document_id, $template, $data = array(), $return = false) {
        ...
        $invite_get =  WP_E_Sig()->validation->esig_clean(esigget("invite")) ;
        ...
        if ($document_id) {
            ...
            $template_data = array(
                ...
                "sender_first_name" => esigget("first_name",$owner),
                "sender_last_name" => esigget("last_name",$owner),
                "owner_email" => esigget("user_email",$owner),
        ...
        if ($return) {
            ...
            WP_E_View::instance()->render($template,"documents", $template_data, false);
        }
    ...
    public function admin_can_view() {
        ...
        $esigpreview = esigget('esigpreview');
        $esig_preview = isset($esigpreview) ? WP_E_Sig()->validation->esig_clean($esigpreview) : NULL;
       
        $docId = esigget('document_id');

        $esig_preview = apply_filters("esig_preview_login_required", $esig_preview , $docId);
        
        if (!is_user_logged_in() && $esig_preview == "1") {
            
            $redirect = wp_login_url() . '?redirect_to=' . urlencode($_SERVER['REQUEST_URI']);
        ...
        $allow = $esigrole->user_can_view_document($docId);
        
        $allow = apply_filters("can_view_preview_document", $allow,$docId);
       
        if (!$allow) {
            return $allow;
        }
        
        if (isset($esig_preview) && $esig_preview == "1" || $esig_preview == "2") {
    ...
    public static function esig_footer_scripts() {
        ...
        // load new signature pad
        $scripts[] = 'esig-signaturepad';

        echo "<script type='text/javascript'>";
        $preview = isset($_GET['esigpreview']) ? $_GET['esigpreview'] : null;
        $ccpreview = esigget("ccpreview");
        if($ccpreview) $preview=1;
        ...
        echo '/* <![CDATA[ */
				var esigAjax = {"ajaxurl":"' . self_admin_url('admin-ajax.php?action=wp_e_signature_ajax&_wpnonce=' . $esigNonce ) . '","preview":"' . $preview . '","document_id":"' . $document_id . '","invite":"' . $invite . '","esig_mobile":"' . $esig_mobile . '","ccpreview":"' . esigget('ccpreview') . '","sign_device":"' . $device . '","esig_mode":"' . $mode . '","site_lang":"'. $site_lang .'"};
			/* ]]> */ 
			</script>';
    ...
    public static function register_scripts() {
        ...
        wp_register_script('esig-core', ESIGN_ASSETS_DIR_URI . "/js/wpesignature.js", array(), esigGetVersion(), true);
        ...
        wp_register_style('esig-safari-style-css', plugins_url('assets/css/safari_mobile.css', dirname(__FILE__)), array(), esigGetVersion(), 'screen');
        ...
        // new signature pad register here 
        wp_register_script('esig-signaturepad', plugins_url("assets/js/signaturepad/signature-pad.js", dirname(__FILE__)), array(), esigGetVersion(), true);
