...
(function ($)
{
    ...
    $(function ()
    {
        ...
        $('body').on('click', '.esig-sif-panel-textfield .insert-btn', function ()
        {
            ...
            // field type checking added .
            let fieldType = "";
            let fieldTypeVal = $("#esig-sif-textfield-option:checked").val();
            if (fieldTypeVal)
            {
                fieldType = 'fieldType="' + fieldTypeVal +'"';
            }
            ...
            var return_text = ' [esigtextfield '+ fieldType +' name="' + name + '" verifysigner="' + verifysigner + '" size="' + maxsize + '" label="' + label + '" displaytype="' + display_type + '" ' + required + ' ] ';
        ...
        $('body').on('click', '.popover .close', function ()
        {
            ...
            $(".sif_textbox_advanced_content #maxsize").attr('value', maxSize);

            let fieldSelected = $('#esig-sif-textfield-option:checked').val();

            if (fieldSelected)
            {
                $(".sif_textbox_advanced_content input[name=esig_textfield_option][value=" + fieldSelected + "]").attr('checked', 'checked');
            }
            $("#sif_textbox_advanced_button").popover('hide');
            $('#sif_textbox_advanced_button').data('bs.popover').options.content = $('.sif_textbox_advanced_content').html();
        ...
        $('body').on('change', '.popover #esig-textarea-size', function () {
            ...
            $("#esig-te20xtarea-input").removeClass();
        ...
        $("#sif_textarea_advanced_button").popover(
                {
                    ...
                    template: '<div id="esig-popup-over" data-id="show" class="popover" role="tooltip"><h3 class="popover-title"></h3><div class="popover-content"></div></div>'
                });
        ...
        $('body').on('click', '.popover .close', function ()
        {
            ...
            $('#sif_textarea_advanced_button').attr('data-id', 'show');
            $('#sif_radio_advanced_button').attr('data-id', 'show');
            $('#sif_checkbox_advanced_button').attr('data-id', 'show');
            setTimeout(() => {  
                $(".sif_others_area").show();
            }, 200);
        });

        $('body').on('click', '#sif_textarea_advanced_button', function (){
            $('.popover').removeAttr("style");
            var PopupData = $('#sif_textarea_advanced_button').attr('data-id');
            if (PopupData == 'show'){

                $(".sif_others_area").hide();
                $('#sif_textarea_advanced_button').attr('data-id', 'hide');
            }if(PopupData == 'hide'){
                $('#sif_textarea_advanced_button').attr('data-id', 'show');
                $(".sif_others_area").show();
            }
        ...
        $('body').on('click', '#sif_radio_advanced_button', function (){
            $('.popover').removeAttr("style");

            var radioPopupData = $('#sif_radio_advanced_button').attr('data-id');
            if(radioPopupData == 'show'){

                $(".sif_others_area").hide();
                $('#sif_radio_advanced_button').attr('data-id', 'hide');
                $('.popover').hide();
            }if(radioPopupData == 'hide'){
                $(".sif_others_area").show();
                $('#sif_radio_advanced_button').attr('data-id', 'show');
                $('.popover').show();
            }
        ...
        $('body').on('click', '#sif_checkbox_advanced_button', function (){
            $('.popover').removeAttr("style");
            var radioPopupData = $('#sif_checkbox_advanced_button').attr('data-id');
            if(radioPopupData == 'show'){

                $(".sif_others_area").hide();
                $('#sif_checkbox_advanced_button').attr('data-id', 'hide');
                $('.popover').hide();
        ...
        $('body').on('click', '.esig-sif-panel-datepicker .insert-date', function () {
            ...
            if (startDate != "" && validStartDate == "Invalid Date") {
                ...
                BootstrapDialog.alert({
                    title: 'WARNING',
                    message: 'Date format is not correct. Allowed format is mm/dd/yyyy.',
            ...
            if ((Date.parse(endDate) <= Date.parse(startDate))) {
                alert('Max date should be greater than Min date.');
                return  false;
            }
        ...
        $('body').on('click', '.esig-sif-panel-file .insert-file', function () {
            ...
            let maxAllowedSize = $("#esig-maximum-file-size").html();
            ...
            if (typeof (file_size) != "undefined" && $.isNumeric(file_size) && file_size<1) {
                $('.esig-sif-panel-textfield #esign-sif-size-msg').show();
                return false;
            }
            //Check for maximum allowed file size.
            if(file_size > parseInt(maxAllowedSize))
            {
                return false;
            }
        ...
        $('#sif-file-advanced-button').on('click', '.popover .close', function ()
        {
            ...
            else if (content > parseInt(maxAllowedSize)) {
                $('.sif_file_advanced_content #max-file-size').css('border', '1px solid red');
                return false;
            }
        ...
        $('body').on('change keyup paste', '.popover #max-file-size', function () {
            ...
            let maxAllowedSize = $("#esig-maximum-file-size").html();
            maxAllowedSize = parseInt(maxAllowedSize);
            ...
            else if(maxsize > maxAllowedSize){
                $(this).css('border', '1px solid red');
                return false;
            } else {
        ...
        $('body').on('click', '.esig-sif-panel-radio .insert-btn', function ()
        {
            ...
            if (!radios) {
                alert("Radio option can not be empty");
                return false;
            }
        ...
        $("body").on("click", ".esig-sif-panel-radio ul li #addRadio", function ()
        {
            $("#radio_html").append("<li id=\"removeradio\">" +
                    "<input type=\"radio\" onclick=\"esigRadioSif(this);\" name=\"esig-radio-sif\"/>" +
        ...
        $('body').on('click', '.esig-sif-panel-checkbox .insert-btn', function ()
        {
            ...
            if (!boxes) {
                alert("Checkbox option can not be empty");
                return false;
            }
...
function esigRadioSif(e){

    if(e.classList.contains('esigChecked')) {
        e.classList.remove('esigChecked');
        e.checked = false;

        const radioHidden = document.getElementsByClassName("hidden_radio");
        for (let i = 0; i < radioHidden.length; i++) {
            radioHidden[i].value='';
        }

    }else{
        const radioButton = document.getElementsByClassName("esigChecked");
        for (let i = 0; i < radioButton.length; i++) {
            radioButton[i].classList.remove('esigChecked');
        }
        e.classList.add('esigChecked');
        e.checked = true;
    }
